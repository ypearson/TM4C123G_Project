    ...
    else
    {
        ...
        while(v->name)
        {
            if(!cstrcmp(argv[1], v->name))
            {
                ...
                {
                    ascii_uint32_to_ascii_hex(&cmd_cf, *((uint16_t*)p));
                    ascii_append_space(&cmd_cf, 16-i);
                    ascii_uint32_to_ascii(&cmd_cf, *((uint16_t*)p));

                }
                else
                {
                    ascii_uint32_to_ascii_hex(&cmd_cf, *((uint8_t*)p));
                    ascii_append_space(&cmd_cf, 16-i);
                    ascii_uint32_to_ascii(&cmd_cf, *((uint8_t*)p));
                }
